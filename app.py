...
def _first_non_empty_line(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        if cleaned:
            return cleaned
    return "Campaign headline pending"
...
def _extract_bullets(text: str, limit: int = 4) -> list[str]:
    ...
    for line in text.splitlines():
        ...
        if len(bullets) >= limit:
            break
    return bullets or [
        "Audience-ready benefit",
        "Offer-led reason to respond",
        "Clear next step",
    ]
...
def _asset_theme(form_data: dict[str, str], strategy: dict[str, Any] | None = None, creative_strategy: dict[str, Any] | None = None) -> dict[str, Any]:
...
def _render_asset_html(content_type: str, result: str, form_data: dict[str, str], strategy: dict[str, Any] | None = None, creative_strategy: dict[str, Any] | None = None) -> str:
    ...
    result_headline = _extract_label_value(result, ("Headline", "Title"), "")
    if content_type == "Pamphlet":
        fallback_headline = creative_strategy.get("campaign_concept") or creative_strategy.get("primary_hook") or (headline_options[0] if headline_options else "")
    else:
        fallback_headline = creative_strategy.get("primary_hook") or (headline_options[0] if headline_options else "")
    headline = _safe_asset_text(fallback_headline or result_headline or _first_non_empty_line(result), "Campaign headline")
    copy_default = (subheadline_options[0] if subheadline_options else creative_strategy.get("creative_angle") or strategy.get("value_proposition") or f"A timely campaign for {topic}.")
    copy = _safe_asset_text(_extract_label_value(result, ("Supporting copy", "Subheadline", "Introduction", "Subcopy", "Copy", "Body Copy"), copy_default))
    cta = _safe_asset_text((cta_options[0] if cta_options else "") or _extract_label_value(result, ("CTA", "Call to action", "Back panel CTA"), str(strategy.get("primary_cta") or "Get started today")))
    bullets = [_safe_asset_text(item[2:].strip() if item.lower().startswith(("✓ ", "✔ ")) else item) for item in _extract_bullets(result, 3)]
    theme = _asset_theme(form_data, strategy, creative_strategy)
    ...
    if content_type == "Banner":
        ...
        asset_body = f"""
        <main class="asset-shell pamphlet-shell"><section class="brochure"><article class="cover panel"><div><p class="eyebrow">{safe['brand']}</p><h1>{safe['headline']}</h1><p>{safe['copy']}</p></div><div class="offer-badge">{safe['offer']}</div>{visual}</article><article class="panel"><p class="section-label">About</p><h2>Built for {safe['audience']}</h2><p>{safe['copy']}</p></article><article class="panel"><p class="section-label">Benefits</p><div class="benefit-grid">{benefit_cards}</div></article><article class="panel"><p class="section-label">Features</p><ul class="feature-list">{feature_items}<li><span>✓</span>{safe['cta']} is the clearest next step.</li></ul></article><article class="panel proof-panel"><p class="section-label">Creative direction</p><h2>{safe['proof']}</h2><p>{safe['topic']}</p></article><article class="panel offer-panel"><p class="section-label">Special offer</p><h2>{safe['offer']}</h2><a class="asset-cta" href="#details">{safe['cta']}</a><div class="qr-box">QR<br>CODE</div><p class="contact-line">Contact: hello@example.com • www.example.com • Your City</p></article></section></main>"""
    else:
        asset_body = f"""
...
def _create_visual_asset(result: str, form_data: dict[str, str], strategy: dict[str, Any] | None = None, creative_strategy: dict[str, Any] | None = None) -> dict[str, str] | None:
    ...
    (GENERATED_DIR / filename).write_text(
        _render_asset_html(content_type, result, form_data, strategy, creative_strategy),
        encoding="utf-8",
    )
...
def _build_prompt(form_data: dict[str, str], strategy: dict[str, Any] | None = None, creative_strategy: dict[str, Any] | None = None) -> Any:
    ...
    try:
        return builder(form_data, strategy, creative_strategy)
    except TypeError:
        try:
...
def analyze() -> str:
    ...
    try:
        ...
        prompt = _build_prompt(form_data, campaign_strategy, creative_strategy)
        result = _run_analysis(prompt)
        asset = _create_visual_asset(str(result), form_data, campaign_strategy, creative_strategy)
    ...
    return render_template("result.html", result=result, form_data=form_data, asset=asset, campaign_strategy=campaign_strategy, creative_strategy=creative_strategy)
