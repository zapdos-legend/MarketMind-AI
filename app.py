...
def _render_asset_html(content_type: str, result: str, form_data: dict[str, str], strategy: dict[str, Any] | None = None, creative_strategy: dict[str, Any] | None = None, design_strategy: dict[str, Any] | None = None, composition_strategy: dict[str, Any] | None = None) -> str:
    ...
    if layout_family == "Fitness / App Launch":
        poster_layout = f"""<main class="asset-shell fitness-shell"><section class="poster-preview fitness-poster"><div class="brand-row"><div class="logo-mark">{escape(brand[:1].upper())}</div><strong>{safe['brand']}</strong><div class="offer-badge">{safe['offer']}</div></div><div class="fitness-grid"><div><p class="eyebrow">Built for {safe['audience']}</p><h1>{safe['headline']}</h1><p class="asset-copy">{safe['copy']}</p><a class="asset-cta neon" href="#">{safe['cta']}</a></div><div class="visual-wrap app-stack">{visual}<div class="float-card one">⚡ Fast sessions</div><div class="float-card two">📈 Track progress</div></div></div><div class="benefit-grid">{benefit_cards}</div></section></main>"""
        banner_layout = f"""<main class="asset-shell"><section class="banner-preview fitness-banner"><div><p class="eyebrow">{safe['brand']} • {safe['offer']}</p><h1>{safe['headline']}</h1></div><div class="banner-center">{banner_visual}<a class="asset-cta neon" href="#">{safe['cta']}</a></div><div class="benefit-rail">{benefit_cards}</div></section></main>"""
        ...
        poster_layout = f"""<main class="asset-shell"><section class="poster-preview cafe-poster"><div class="cafe-plate">{visual}</div><div class="cafe-copy"><p class="eyebrow">{safe['brand']}</p><h1>{safe['headline']}</h1><p class="asset-copy">{safe['copy']}</p><div class="price-badge">{safe['offer']}</div><a class="asset-cta" href="#">{safe['cta']}</a></div></section></main>"""
        banner_layout = f"""<main class="asset-shell"><section class="banner-preview cafe-banner"><div class="price-badge">{safe['offer']}</div><div><p class="eyebrow">Morning made easy</p><h1>{safe['headline']}</h1><p class="asset-copy">{safe['copy']}</p><a class="asset-cta" href="#">{safe['cta']}</a></div>{banner_visual}</section></main>"""
        ...
        poster_layout = f"""<main class="asset-shell"><section class="poster-preview luxury-poster"><div class="luxury-hero">{visual}</div><div class="luxury-copy"><p class="eyebrow">{safe['brand']}</p><h1>{safe['headline']}</h1><div class="gold-line"></div><p class="asset-copy">{safe['copy']}</p><a class="asset-cta" href="#">{safe['cta']}</a><span class="trust-pill">{safe['offer']}</span></div></section></main>"""
        banner_layout = f"""<main class="asset-shell"><section class="banner-preview luxury-banner"><div>{banner_visual}</div><div><p class="eyebrow">{safe['brand']}</p><h1>{safe['headline']}</h1><div class="gold-line"></div><p>{safe['offer']}</p><a class="asset-cta" href="#">{safe['cta']}</a></div></section></main>"""
        ...
        poster_layout = f"""<main class="asset-shell"><section class="poster-preview healthcare-poster"><div class="trust-strip">✓ Trusted care • Easy appointments • Calm support</div><div class="poster-hero"><div><p class="eyebrow">{safe['brand']}</p><h1>{safe['headline']}</h1><p>{safe['copy']}</p><a class="asset-cta" href="#">{safe['cta']}</a></div>{visual}</div><div class="benefit-grid">{benefit_cards}</div></section></main>"""
        banner_layout = f"""<main class="asset-shell"><section class="banner-preview healthcare-banner"><div><p class="eyebrow">{safe['brand']}</p><h1>{safe['headline']}</h1><p>{safe['copy']}</p><a class="asset-cta" href="#">{safe['cta']}</a></div><div class="trust-strip">{safe['proof']}</div>{banner_visual}</section></main>"""
...
def _create_visual_asset(result: str, form_data: dict[str, str], strategy: dict[str, Any] | None = None, creative_strategy: dict[str, Any] | None = None, design_strategy: dict[str, Any] | None = None, composition_strategy: dict[str, Any] | None = None) -> dict[str, str] | None:
    ...
    (GENERATED_DIR / filename).write_text(
        _render_asset_html(content_type, result, form_data, strategy, creative_strategy, design_strategy, composition_strategy),
        encoding="utf-8",
    )
...
def _build_prompt(form_data: dict[str, str], strategy: dict[str, Any] | None = None, creative_strategy: dict[str, Any] | None = None, design_strategy: dict[str, Any] | None = None, composition_strategy: dict[str, Any] | None = None) -> Any:
    ...
    try:
        return builder(form_data, strategy, creative_strategy, design_strategy, composition_strategy)
    except TypeError:
        try:
...
def analyze() -> str:
    ...
    try:
        ...
        prompt = _build_prompt(form_data, campaign_strategy, creative_strategy, design_strategy, composition_strategy)
        result = _run_analysis(prompt)
        asset = _create_visual_asset(str(result), form_data, campaign_strategy, creative_strategy, design_strategy, composition_strategy)
    ...
    return render_template("result.html", result=result, form_data=form_data, asset=asset, campaign_strategy=campaign_strategy, creative_strategy=creative_strategy, design_strategy=design_strategy, composition_strategy=composition_strategy)
