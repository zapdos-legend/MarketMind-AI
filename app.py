...
def _asset_theme(form_data: dict[str, str], strategy: dict[str, Any] | None = None) -> dict[str, Any]:
...
def _render_asset_html(content_type: str, result: str, form_data: dict[str, str], strategy: dict[str, Any] | None = None) -> str:
    ...
    strategy = strategy or {}
    offer = _safe_asset_text(strategy.get("offer_strategy") or form_data.get("offer"), "Limited-time offer")
    ...
    cta = _safe_asset_text(_extract_label_value(result, ("CTA", "Call to action", "Back panel CTA"), str(strategy.get("primary_cta") or "Get started today")))
    bullets = [_safe_asset_text(item[2:].strip() if item.lower().startswith(("✓ ", "✔ ")) else item) for item in _extract_bullets(result, 3)]
    theme = _asset_theme(form_data, strategy)
...
def _create_visual_asset(result: str, form_data: dict[str, str], strategy: dict[str, Any] | None = None) -> dict[str, str] | None:
    ...
    (GENERATED_DIR / filename).write_text(
        _render_asset_html(content_type, result, form_data, strategy),
        encoding="utf-8",
    )
...
def _build_prompt(form_data: dict[str, str], strategy: dict[str, Any] | None = None) -> Any:
...
def analyze() -> str:
    ...
    try:
        ...
        prompt = _build_prompt(form_data, campaign_strategy)
        result = _run_analysis(prompt)
        asset = _create_visual_asset(str(result), form_data, campaign_strategy)
    ...
    return render_template("result.html", result=result, form_data=form_data, asset=asset, campaign_strategy=campaign_strategy)
