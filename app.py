...
def _asset_theme(form_data: dict[str, str]) -> dict[str, str]:
    """Choose a professional visual theme from campaign details."""
    combined = " ".join(form_data.get(key, "") for key in ("topic", "business_name", "product_service", "audience", "offer", "notes")).lower()
    theme_list = [
        (("fitness", "gym", "workout", "health", "training", "exercise"), "Momentum Fitness", "#111827", "#16a34a", "#f97316", "#dcfce7", "linear-gradient(135deg, #0f172a 0%, #14532d 52%, #f97316 100%)", "https://source.unsplash.com/900x1100/?fitness,gym,workout", ("⚡", "📈", "🏋️"), "Trusted by students building stronger daily routines"),
        (("pizza", "restaurant", "food", "meal", "dining", "delivery", "weekend"), "Hot Table", "#7c2d12", "#ef4444", "#facc15", "#ffedd5", "linear-gradient(135deg, #7f1d1d 0%, #ea580c 55%, #facc15 100%)", "https://source.unsplash.com/900x1100/?pizza,restaurant,food", ("🍕", "👨‍👩‍👧", "🚚"), "Loved by families, students, and weekend foodies"),
        (("iit", "jee", "coaching", "academy", "class", "student", "education", "lecture"), "Scholastic Edge", "#312e81", "#2563eb", "#a855f7", "#e0e7ff", "linear-gradient(135deg, #1e1b4b 0%, #2563eb 56%, #a855f7 100%)", "https://source.unsplash.com/900x1100/?classroom,students,education", ("🎓", "📚", "💬"), "Trusted by ambitious Class 11 and 12 students"),
        (("skin", "skincare", "beauty", "cosmetic", "glow", "premium", "kit"), "Soft Glow", "#831843", "#db2777", "#f9a8d4", "#fce7f3", "linear-gradient(135deg, #831843 0%, #db2777 52%, #f9a8d4 100%)", "https://source.unsplash.com/900x1100/?skincare,beauty,cosmetics", ("✨", "🌿", "💧"), "Loved by Gen Z beauty buyers and glow seekers"),
        (("app", "technology", "software", "digital", "ai", "platform", "saas"), "Digital Launch", "#0f172a", "#0284c7", "#22d3ee", "#cffafe", "linear-gradient(135deg, #0f172a 0%, #0369a1 52%, #22d3ee 100%)", "https://source.unsplash.com/900x1100/?app,technology,digital", ("🚀", "📱", "🔒"), "Built for modern customers who expect fast digital experiences"),
    ]
    for keywords, name, primary, secondary, accent, soft, gradient, image, icons, proof in theme_list:
        if any(keyword in combined for keyword in keywords):
            return {"name": name, "primary": primary, "secondary": secondary, "accent": accent, "soft": soft, "gradient": gradient, "image": image, "icons": icons, "proof": proof}
    return {"name": "Agency Classic", "primary": "#3730a3", "secondary": "#4f46e5", "accent": "#06b6d4", "soft": "#eef2ff", "gradient": "linear-gradient(135deg, #312e81 0%, #4f46e5 55%, #06b6d4 100%)", "image": "https://source.unsplash.com/900x1100/?marketing,business,brand", "icons": ("⭐", "📣", "✅"), "proof": "Trusted by customers who want clear value and confident next steps"}


def _icon_card_html(items: list[str], icons: tuple[str, ...]) -> str:
    """Render benefit cards with icons."""
    return "".join(f'<article class="benefit-card"><span>{escape(icons[index % len(icons)])}</span><strong>{escape(item)}</strong></article>' for index, item in enumerate(items[:3]))
...
def _render_asset_html(content_type: str, result: str, form_data: dict[str, str]) -> str:
    ...
    copy = _extract_label_value(result, ("Supporting copy", "Subheadline", "Introduction", "Subcopy", "Copy", "Body Copy"), f"A polished campaign preview for {topic}.")
    cta = _extract_label_value(result, ("CTA", "Call to action", "Back panel CTA"), "Get started today")
    bullets = [item[2:].strip() if item.lower().startswith(("✓ ", "✔ ")) else item for item in _extract_bullets(result, 3)]
    theme = _asset_theme(form_data)
    safe = {key: escape(value) for key, value in {"content_type": content_type, "topic": topic, "brand": brand, "offer": offer, "audience": audience, "headline": headline, "copy": copy, "cta": cta, "result": result, "image": theme["image"], "proof": theme["proof"], "theme_name": theme["name"]}.items()}
